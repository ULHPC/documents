...
% Time-stamp: <Fri 2015-06-26 13:50 svarrette>
% 
...
\frame{
  \frametitle{Memory bottleneck} %Last Challenges \hfill{\small for a better efficiency}}

\centering\includegraphics[width=0.6\textwidth]{memory_vs_cpu.pdf}

  
  \begin{itemize}
    \item A regular computing node have at least 2GB/core RAM
      \begin{itemize}
          \itemhook Do 12-24 runs fit in the memory?
          \itemhook If your job runs out of memory, it simply crashes
      \end{itemize}
    \item Use fewer simultaneous runs if \textbf{really} needed!
      \begin{itemize}
          \itemhook \textbf{OR} request a big memory machine (1TB RAM)\\
          \begin{cmdline}
              \cmdlineentry{oarsub -t bigmem ...}
          \end{cmdline}
          \itemhook \textbf{Or} explore parallization (MPI, OpenMP, pthreads)
      \end{itemize}
      % \item Use \texttt{\$SCRATCH} directory whenever you can
      %   \begin{itemize}
      %       \itemhook \texttt{gaia}: \textbf{shared} between nodes (Lustre FS)
      %       \itemhook \texttt{chaos}: \textbf{NOT shared}
      %       (\texttt{/tmp}) and clean at job end.
      %   \end{itemize}
  \end{itemize}
  % \end{block}
}

\subsection{Effective Storage and Memory Management}
      ...
      \begin{itemize}
        \item \textbf{Shared} storage
          \begin{itemize}
              ...
              \itemhook \texttt{GPFS} -- \emph{scalable}  ~~$\simeq$ 3 GB/s   (R) \hfill $\mathcal{O}$(500 TB)
              \itemhook \texttt{Lustre} -- \emph{scalable}  ~~$\simeq$ 3 GB/s   (R) \hfill $\mathcal{O}$(400 TB)
          \end{itemize}
...
\begin{frame}[t]
    ...
    \begin{itemize}
      ...
      \item<+-> Reliability vs. Crash Faults in Distributed systems
        \only<2>{
          \begin{center}
              \includegraphics[width=0.65\textwidth]{proba_defaillance}
          \end{center}
        }
